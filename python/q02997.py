...
class Solution:
    def minOperations(self, nums: List[int], k: int) -> int:
        ...
        x = 0  # XOR identity element
        for n in nums:
            x ^= n  # XOR is associative
        return (k ^ x).bit_count()  # count bits that differ between `x` and `k`
    ...
    def minOperations(self, nums: List[int], k: int) -> int:
        return functools.reduce(operator.xor, nums, k).bit_count()  # one-liner